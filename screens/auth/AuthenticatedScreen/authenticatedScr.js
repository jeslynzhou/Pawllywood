...
const AuthenticatedScreen = ({ user, navigateToProfile }) => {
  ...
  // email text
  const emailFontSize = height * 0.025; // size of email text (adjusted for more appropriate size)
  // image
  ...
  return (
    <View style={styles.container}>
      <View style={[styles.titleContainer, { marginTop: titleCmarginTop }]}>
        ...
        <Text style={[styles.emailText, { fontSize: emailFontSize, lineHeight: emailFontSize, marginVertical }]}>{user.email}</Text>
      </View>
      <Image
